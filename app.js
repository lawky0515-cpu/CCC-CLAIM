...
document.getElementById('sync-cloud').addEventListener('click',async()=>{
 ...
 button.disabled=true;message.textContent='正在检查 Google Sheets…';
 try{
  const result=await synchronizeAll();
  message.textContent='同步完成并核对：共 '+result.total+' 笔记录。';
 }catch(error){message.textContent='同步未完成：'+error.message+' 本机记录未删除；可重试。';}
