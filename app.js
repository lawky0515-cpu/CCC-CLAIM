...
async function claimCloudList(){
 ...
 try{
  ...
  const response=await Promise.race([
   ...
   new Promise((_,reject)=>setTimeout(()=>reject(Error('GET 读取超时。')),6000))
  ]);
...
async function refreshCloudRecords(quiet=false){
 ...
 cloudRefreshPromise=(async()=>{
  try{
   ...
   let remote=migrateStatuses(await claimCloudList());
   ...
   const failed=[];
   let uploaded=false;
   for(const record of pending){try{await syncOneRecord(record);uploaded=true;}catch{failed.push(record);}}
   if(uploaded){
    try{remote=migrateStatuses(await claimCloudList());}catch{}
   }
   const remoteIds=new Set(remote.map(record=>record.id));
   const localOnly=failed.filter(record=>!remoteIds.has(record.id));
   const visible=[...localOnly,...remote];
   if(!persist(visible))throw Error('云端已读取，但本机缓存更新失败。');
   message.textContent=localOnly.length
    ? '已加载 '+remote.length+' 笔云端记录；另有 '+localOnly.length+' 笔本机记录等待重试。'
    : '已从 Google Sheets 加载 '+remote.length+' 笔记录。';
   return visible;
  }catch(error){
